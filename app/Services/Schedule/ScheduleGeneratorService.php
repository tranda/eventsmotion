...
class ScheduleGeneratorService
{
    ...
    private function generateForDiscipline(
        ...
        // Combined categories: a secondary never creates its own races — its
        // crews are generated by the host's pass. The host races the merged
        // field as one shared Final (scored per category later). Emit a note so
        // the organizer sees WHY this category has no separate race (otherwise
        // it just looks like a missing race).
        if ($discipline->isCombinedSecondary()) {
            $host = $discipline->combinedWith;
            $hostName = $host ? $host->getDisplayName() : 'another category';
            $result->addWarning(
                "{$discipline->getDisplayName()}: racing combined with {$hostName} — no separate race."
            );
            return;
        }
        $isCombinedHost = $discipline->isCombinedHost();

        $crews = $isCombinedHost
            ? $this->combinedCrewsFor($discipline)
            : $discipline->crews()->orderBy('id')->get();
        ...
        // A combined group always races as one shared Final holding both
        // categories' crews — never rounds/heats/flights (keeps per-category
        // scoring simple and fair). Overrides/long-distance rules don't apply.
        if ($isCombinedHost) {
            $this->generateCombinedFinal($discipline, $laneCount, $crews, $result);
            return;
        }
    ...
    /**
     * The union of crews across a combined group (host + secondaries), ordered
     * by discipline then id so the layout is deterministic. Each crew keeps its
     * own discipline_id, which is what drives per-category scoring later.
     */
    private function combinedCrewsFor(Discipline $host)
    {
        $memberIds = $host->combinedMembers()->pluck('id')->all();
        return Crew::whereIn('discipline_id', $memberIds)
            ->orderBy('discipline_id')
            ->orderBy('id')
            ->get();
    }
    ...
    /**
     * Generate a single shared "Final" for a combined group — one race holding
     * every category's crews together. Lanes are centre-out when the merged
     * field fits the course, sequential otherwise. discipline_id is the host;
     * each CrewResult's crew carries its own category for separate scoring.
     */
    private function generateCombinedFinal(
        Discipline $host,
        int $laneCount,
        $crews,
        GenerationResult $result,
        ...
        $ordered = $crews
            ->sortBy(fn ($c) => sprintf('%011d%011d', (int) $c->discipline_id, (int) $c->id))
            ->values();
        $count = $ordered->count();
        $laneOrder = $count <= $laneCount ? $this->centreOutLaneOrder($laneCount) : null;

        foreach ($ordered as $idx => $crew) {
            $lane = $laneOrder !== null ? $laneOrder[$idx] : $idx + 1;
            CrewResult::create([
                'race_result_id' => $race->id,
                'crew_id' => $crew->id,
                'lane' => $lane,
        ...
        $result->racesPerDiscipline[$host->id] = 1;
    }
    ...
    /**
     * Centre-out lane order per IDBF: lanes ordered by distance from the course
     * centre ((laneCount+1)/2), and when two lanes are equally central the LOWER
     * lane comes first — so a short field fills the near/low lanes rather than
     * the outside. Even counts are unchanged (4 → [2,3,1,4]; 6 → [3,4,2,5,1,6];
     * 8 → [4,5,3,6,2,7,1,8]); odd counts now prefer the lower neighbour
     * (3 → [2,1,3] so 2 crews sit in lanes 1,2; 5 → [3,2,4,1,5]).
     *
     * @return list<int> lanes in centre-out order
     */
    private function centreOutLaneOrder(int $laneCount): array
    {
        // Geometric centre: an integer lane for odd counts, a .5 midpoint for
        // even counts (so both middle lanes are equidistant). The .5 values
        // (0.5, 1.5, …) are exact in binary float, so the equality tie-break holds.
        $centre = ($laneCount + 1) / 2.0;
        $lanes = range(1, $laneCount);
        usort(
            $lanes,
            fn($a, $b) => (abs($a - $centre) <=> abs($b - $centre)) ?: ($a <=> $b),
        );
        return $lanes;
    }
